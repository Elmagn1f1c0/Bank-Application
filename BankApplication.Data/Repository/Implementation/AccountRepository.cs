...
namespace BankApplication.Data.Repository.Implementation
{
    public class AccountRepository : IAccountRepository
    {
        ...
        {
            var accountToBeUpdated = _db.Accounts.Find(account.Id); // Find by ID
            if (accountToBeUpdated == null)
            {
                throw new ApplicationException("Account does not exist");
            }

            // Check if you want to update the email
            if (!string.IsNullOrWhiteSpace(account.Email))
            {
                // Check if the email you are trying to change to is already taken
